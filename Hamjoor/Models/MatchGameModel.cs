...
namespace Hamjoor.Models
{
    public class MatchGameModel
    {
        ...
        public MatchGameModel(int turnDelayDuration, int cardCount)
        {
            this.turnDelayDuration = turnDelayDuration;
            this.cardCount = cardCount;
            animalEmojis = animals.Take(cardCount).ToList();
            this.ShuffledCards = new List<AnimalCard>();
            ResetGame();
        }
        ...
        public async Task SelectCardAsync(AnimalCard card)
        {
            ...
            // Is the game won?
            if (MatchesFound == animalEmojis.Count)
            {
